		...
		<tr>
			<td style="padding-right: 30px;">
				...
				<select id="section_id" onchange="Get_Module_Options()">
					...
					<?php
					$Sections = mysqli_query($_SESSION['connection'],"SELECT DISTINCT(section.id), section.name FROM `order` JOIN job ON order.id = job.order_id
			...
			<td style="padding-right: 30px;">
				...
				<select id="customer_id" onchange="Get_Module_Options()">
					<option value="">All</option>
					<?php 
					$Customers = mysqli_query($_SESSION['connection'],"SELECT DISTINCT(customer.id), customer.name FROM `order` JOIN job ON order.id = job.order_id
					JOIN customer ON order.customer_id = customer.id
					JOIN product ON product.id = job.product_id
					JOIN machine ON machine.id = job.machine_id
					JOIN machine_assignment ON machine_assignment.machine_id=machine.id
			...
			<td style="padding-right: 30px;">
				...
				<select id="order_id" onchange="Get_Module_Options()">
					<option value="">All</option>
					<?php
					$Orders = mysqli_query($_SESSION['connection'],"SELECT DISTINCT(order.id), order.number FROM `order` JOIN job ON order.id = job.order_id
					JOIN customer ON order.customer_id = customer.id
					JOIN product ON product.id = job.product_id
					JOIN machine ON machine.id = job.machine_id
					JOIN machine_assignment ON machine_assignment.machine_id=machine.id
			...
			<td style="padding-right: 30px;">	
				...
				<select id="description" onchange="Get_Module_Options()">
					<option value="">All</option>
					<?php
					$Products = mysqli_query($_SESSION['connection'],"SELECT DISTINCT(product.description) FROM `order` JOIN job ON order.id = job.order_id
					JOIN customer ON order.customer_id = customer.id
					JOIN product ON product.id = job.product_id
					JOIN machine ON machine.id = job.machine_id
					JOIN machine_assignment ON machine_assignment.machine_id=machine.id
			...
			<td style="padding-right: 30px;">
				...
				<select id="product_id" onchange="Get_Module_Options()">
					<option value="">All</option>
					<?php
					$Drawing_numbers = mysqli_query($_SESSION['connection'],"SELECT DISTINCT(product.id), product.drawing_number FROM `order` JOIN job ON order.id = job.order_id
					JOIN customer ON order.customer_id = customer.id
					JOIN product ON product.id = job.product_id
					JOIN machine ON machine.id = job.machine_id
					JOIN machine_assignment ON machine_assignment.machine_id=machine.id
					...
					WHERE ".str_replace("=''", "!=''", "section.id='".$_POST['section_id']."' && customer.id='".$_POST['customer_id']."' && order.id='".$_POST['order_id']."' && product.description='".$_POST['description']."' && product.id='".$_POST['product_id']."'")."
					ORDER BY product.id ASC");
					while($Drawing_number = mysqli_fetch_assoc($Drawing_numbers))
