<?php
	...
	$Allmachines = mysql_query("SELECT machine.id,machine.machine_number  FROM machine where machine.id not in(select `job`.machine_id from job  WHERE (job.tentative_date >='".date('Y-m-d', strtotime($_GET['startdate']))."' and job.tentative_enddate <='".date('Y-m-d', strtotime($_GET['enddate']))."')) ");
	echo'<select id="machine_id" name="machine_id">
		<option value="">Select</option>';
			while($Machines = mysql_fetch_assoc($Allmachines))
			{
