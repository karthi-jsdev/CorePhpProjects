<section role="main" id="main">
	<?php
		...
		if($_GET['action'] == 'Edit')
		{
			$Class = mysql_fetch_assoc(Payment_Select_ById());
			foreach($Columns as $Col)
				$_POST[$Col] = $Class[$Col];
		}
		else if($_GET['action'] == 'Delete')
		{
			Payment_Delete_ById($_GET['id']);
			$message = "<br /><div class='message success'><b>Message</b> : One Payment Mode deleted successfully</div>";
		}
		
		if(isset($_POST['Submit']) || isset($_POST['Update']))
		{
			$ClassResource = Payment_Select_ByNamePWD();
			if(isset($_POST['Submit']))
			{
				if(mysql_num_rows($ClassResource))
					$message = "<br /><div class='message error'><b>Message</b> : This Payment Mode already exists</div>";
				else
				{
					Payment_Insert();
					$message = "<br /><div class='message success'><b>Message</b> : Payment Mode added successfully</div>";
				}
			}
			else if(isset($_POST['Update']))
			{
				$Class = mysql_fetch_assoc($ClassResource);
				if(mysql_num_rows(Payment_Select_ByNamePWDId()))
					$message = "<br /><div class='message error'><b>Message</b> : This Payment Mode already exists</div>";
				else
				{
					Payment_Update();
					$message = "<br /><div class='message success'><b>Message</b> : Payment Mode details updated successfully</div>";
				}
			}
			foreach($Columns as $Col)
				$_POST[$Col] = "";
	...
	<div class="columns" style='width:902px;'>
		...
		<form method="post" action="?page=<?php echo $_GET['page']."&subpage=".$_GET['subpage']."&innersubpage=".$_GET['innersubpage']."&pageno=".$_GET['pageno']; ?>" id="form" class="form panel" onsubmit="return validation()">
			...
			<header><h2>Add Payment Mode</h2></header>
			<hr />				
			<fieldset>
				<div class="clearfix">
		...
		<div class="columns">
			<h3>Payment Mode List
				<?php
				$PaymentTotalRows = mysql_fetch_assoc(Payment_Select_Count_All());
				echo " : No. of Total Payment Mode - ".$PaymentTotalRows['total'];
				?>
			</h3>
			<hr />			
			<table class="paginate sortable full">
				...
				<tbody>
					...
					$PaymentRows = Payment_Select_ByLimit($Start, $Limit);
					while($Payment = mysql_fetch_assoc($PaymentRows))
					{
