...
<?php
	...
	{		
		Biomedical_Assets_Status_Insert($_POST['id'],$_POST['description'],$_POST['statusid']);			
	}
	if($_GET['id'])
	{
?>
<section class="grid_6 first">
	<form id="form" class="form panel" method="POST" onsubmit="return validation()">
		<header><h2>Biomedical Asset Status</h2></header>
		<hr />
		...
		<fieldset>
			<div class="clearfix">
				...
				<select  id='statusid' name='statusid'>
					...
					<?php 		
					$Status_Select = Biomedical_Complaint_Get_Status();
	...
	<?php
			$AssetComplaint = Biomedical_Select_All_Complaint($_GET['id']);
	...
	<div class="columns leading">
		<div class="grid_6 first">
			<h3>Biomedical Assets List
			<?php
			$AssetsTotalRows = mysqli_num_rows(Biomedical_Complaint_Get_Status_Select_ById($_GET['id']));
			echo " : No. of total Biomedical Asset Status - ".$AssetsTotalRows;
			?>
			</h3>
			<hr />
				<table class="paginate sortable full">
				...
				<tbody>
					...
					$Assets_Status_Description = Biomedical_Assets_Status_Select_ById($_GET['id']);
					while($Asset_Status = mysqli_fetch_assoc($Assets_Status_Description))
					{
						echo "<tr>
							<td align='center'>".$i++."</td>";
							echo "<td>".$Asset_Status['biomedicalassetdescription']."</td>";
							$Status = mysqli_fetch_assoc(Biomedical_Complaint_Get_Status_Select_ById($Asset_Status['statusid']));
...
<section class="grid_6 first">
	<div class="columns leading">
		...
		<table class="paginate sortable full">				
			...
			<tbody>
				<?php
				$AssetsTotalRows = mysqli_num_rows(Biomedical_Assets_Status_Select_ByName($_POST['ItemName']));
				if(!$AssetsTotalRows)
				...
				$Status = array("<a href='#' class='action-button' title='delete'><span class='delete'></span></a>", "<a href='#' class='action-button' title='accept'><span class='accept'></span></a>");
				if($_POST['ItemName'])
					$AssetsRows = Biomedical_Assets_Status_Select_ByItemName($_POST['ItemName'],$Start, $Limit);
				else
					$AssetsRows = Biomedical_Assets_Select_ByLimit($Start, $Limit);
