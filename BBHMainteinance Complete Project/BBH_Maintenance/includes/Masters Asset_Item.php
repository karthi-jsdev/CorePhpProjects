<section class="grid_6 first">
	<?php
		...
		$_POST['name'] = $_POST['assetname'];
		if($_GET['action'] == 'Edit')
		{
			$Item = mysqli_fetch_assoc(System_Select_ById($_GET['id']));
		...
		if(isset($_POST['Submit']) || isset($_POST['Update']))
		{
			if(mysqli_num_rows(System_Select_ByName($_POST['name'])))
				$message = "<br /><div class='message error'><b>Message</b> : This Item already exists</div>";
			else
			{
			...
			<form method="post" action="?page=<?php echo $_GET['page']."&subpage=".$_GET['subpage']."&pageno=".$_GET['pageno']; ?>" id="form" class="form panel" onsubmit="return validation()">
				...
				<fieldset>
					<div class="clearfix">
						...
						<input type="text" name="assetname" id="assetname" required="required" value="<?php echo $_POST['assetname']; ?>" onkeypress="return isAlphaOrNumeric(event)"/>
                    </div>
				</fieldset>
				<hr />
				<?php
	...
	<div class="columns leading">
		<div class="grid_6 first">
			<h3>Item List
			<?php
			$ItemTotalRows = mysqli_num_rows(System_Select_All());
			echo " : No. of total Items - ".$ItemTotalRows;
			?>
			</h3>
			<hr />			
...
<script>
	...
	{
		var message = "";
		if(document.getElementById('assetname').value.length < 2 || document.getElementById('assetname').value.length > 15)
			message = "Item Name should be within 2 to 15 characters";
		if(message)
		{
			alert(message);
			return false;
