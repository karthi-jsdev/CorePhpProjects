	...
	<div class="columns" style='width:902px;'>
		...
		<form method="post" action="?page=<?php echo $_GET['page']."&subpage=".$_GET['subpage']."&innersubpage=".$_GET['innersubpage']."&pageno=".$_GET['pageno']; ?>" id="form" class="form panel" onsubmit="return validation()">
			...
			<fieldset>
				<div class="clearfix">
					...
					{
					?>
						<input type="text" autocomplete="off" id="type" name="type" required="required" style="background-color:#E0E0E0;" readonly value="<?php echo $_POST['type']; ?>" onkeypress="return AlphaNumCheck(event)"/>
					<?php }
					else
					{ ?>
						<input type="text" autocomplete="off" id="type" name="type" required="required" value="<?php echo $_POST['type']; ?>" onkeypress="return AlphaNumCheck(event)"/>
				<?php	}	
					?>
				</div>
				<div class="clearfix">
					<label>Percent <font color="red">*</font></label>
					<input type="text" autocomplete="off" id="percent"  name="percent" required="required" value="<?php echo $_POST['percent']; ?>" onkeypress="return percentage(event)"/>
				</div>
				<div class="clearfix">
		...
		<div class="columns">
			<h3>
				...
				echo "Total No. of Taxes - ".$TaxTotalRows['total'];
				?>
			</h3>
			<hr />			
