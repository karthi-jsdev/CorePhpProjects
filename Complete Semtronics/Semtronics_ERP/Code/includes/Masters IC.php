<section role="main" id="main">
	...
	<div class="columns" style='width:902px;'>
		...
		<form method="post" action="?page=<?php echo $_GET['page']."&subpage=".$_GET['subpage']."&innersubpage=".$_GET['innersubpage']."&pageno=".$_GET['pageno']; ?>" id="form" class="form panel">
			...
			<header><h2>IC</h2></header>
			<hr />				
			<fieldset>
				<div class="clearfix">
					<label>IC<font color="red">*</font></label>
						<input type="text" autocomplete="off" id="ic" name="ic" required="required" value="<?php echo $_POST['ic']; ?>" onkeypress="return isAlphabetic(event)"/>
				</div>
				<div class="clearfix">
					...
					<?php
						...
						while($edisable = mysql_fetch_assoc($enabledisable))
						{
							echo'<tr>
									...
									<td><a href="?page=Masters&subpage=Stockmaster&innersubpage=IC&id='.$edisable['id'].'&action=Edit" class=action-button title=user-edit><span class=user-edit></span><a href="?page=Masters&subpage=Stockmaster&innersubpage=IC&id='.$edisable['id'].'&action=Delete"class=action-button title=user-delete onclick="return deleterows();"><span class=user-delete></span></td>
								</tr>';
						}
					?>
				</tbody>
...
<script>
	...
	function validation()
	{
		if(document.getElementById("ic").value==""||document.getElementById("ic").value==null)
