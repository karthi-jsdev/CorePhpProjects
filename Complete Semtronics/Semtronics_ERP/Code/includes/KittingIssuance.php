<?php
	...
	/* if($_GET['getid'])
	{
		...
		//$Stock = mysqli_fetch_assoc(mysqli_query($_SESSION['connection'],"SELECT *,sum(stockinventory.quantity) as qty FROM stock join stockinventory on stockinventory.batchid=stock.batchid WHERE stockinventory.batchid='".$_POST['batchid']."' && stockinventory.inspection='1' group by stockinventory.batchid"));
	...
	{
		$stockqty = mysqli_fetch_assoc(mysqli_query($_SESSION['connection'],"SELECT sum(stockinventory.quantity) as qty FROM stock join stockinventory on stockinventory.batchid=stock.batchid WHERE stockinventory.batchid='".$_POST['batchid']."' && stockinventory.inspection='1' group by stockinventory.batchid"));
		...
		if($_POST['ids'])
		{
			...
			else
			{
				...
				else
				{
					...
					mysqli_query($_SESSION['connection'],"INSERT INTO stockissuance VALUES('', '".$_SESSION['IssuanceId']."', '".$_POST['batchid']."', '".$_POST['quantity']."', '".$Stock['unitprice']."', '".($Stock['unitprice']*$_POST['quantity'])."', '".$Stock['taxid']."', '".(($Stock['taxamount']/$Stock['quantity'])*$_POST['quantity'])."', '".$_POST['issuedto']."')"); 
				}
			}
		}
		else
...
<?php
	...
	$kittinglist = mysqli_query($_SESSION['connection'],"SELECT stockissuance.id as siid,materialcode,rawmaterial.id,productbom.quantity as qty,productbom.productcode,sum(stockinventory.quantity) as quantity,sum(stockissuance.quantity) as squantity
									FROM rawmaterial 
