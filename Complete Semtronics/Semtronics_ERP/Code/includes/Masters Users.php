		...
		<form method="post" action="?page=<?php echo $_GET['page']."&subpage=".$_GET['subpage']."&innersubpage=".$_GET['innersubpage']."&pageno=".$_GET['pageno']; ?>" id="form" class="form panel" onsubmit="return validation()">
			...
			<fieldset>
				<div class="clearfix">
					...
					<input type="text" id="firstname" name="firstname" required="required" autocomplete="off" value="<?php echo $_POST['firstname']; ?>" onkeypress="return isAlphabetic(event)"/>
				</div>
				<div class="clearfix">
					<label>Last Name <font color="red">*</font></label>
					<input type="text" id="lastname"  name="lastname" required="required" autocomplete="off" value="<?php echo $_POST['lastname']; ?>" onkeypress="return isAlphabetic(event)"/>
				</div>
				<div class="clearfix">
					<label>Phone No. <font color="red">*</font></label>
					<input type="text" id="phone" name="phone" required="required" autocomplete="off" value="<?php echo $_POST['phone']; ?>" onkeypress="return isNumeric(event)"/>
				</div>
				<div class="clearfix">
					<label>User Name <font color="red">*</font></label>
					<input type="text" id="name" name="name" required="required" autocomplete="off" value="<?php echo $_POST['name']; ?>" onkeypress="return AlphaNumCheck(event)"/>
				</div>
				<div class="clearfix">
					<label>Password <font color="red">*</font></label>
					<input type="password" id="password" name="password" required="required" autocomplete="off" value="<?php echo $_POST['password']; ?>" onkeypress="return AlphaNumCheck(event)"/><font color="red">Alpha numeric characters with symbols like @ _ .</font>
				</div>
				<div class="clearfix">
					<label>Modules</label>
					<table>
				...
				<?php
					...
					while($ss = mysqli_fetch_assoc($s))
					{
						...
						if($_GET['id'])
						{
							...
							if(in_array($ss['id'],$status))
								echo '<td><input style="display:block;float:left;width:100%;" type="checkbox" id="status" name="status[]" autocomplete="off" value="'.$ss['id'].'" checked />'.$ss['modulename'].'</td>';
							else
								echo '<td><input style="display:block;float:left;width:100%;" type="checkbox" id="status" name="status[]" autocomplete="off" value="'.$ss['id'].'"/>'.$ss['modulename'].'</td>';
						}
						else
							echo '<td><input style="display:block;float:left;width:100%;" type="checkbox" id="status" name="status[]" autocomplete="off" value="'.$ss['id'].'"/>'.$ss['modulename'].'</td>';
						$i++;
...
<script>
	function isAlphabetic(evt)
	{
		...
		if(charCode == 8 || charCode == 32 || charCode == 9 || (charCode>=35 && charCode<=39) || charCode==118 || charCode==46 || charCode==99)
			return true;
	...
	function Reset()
	{
		document.location.assign("index.php?page=<?php echo $_GET['page']; ?>&subpage=<?php echo $_GET['subpage']; ?>");
	}
</script>