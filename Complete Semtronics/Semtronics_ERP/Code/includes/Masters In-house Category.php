<section role="main" id="main">
	<?php
		...
		{
			Inhousecategory_Delete_ById($_GET['id']);
			$message = "<br /><div class='message success'><b>Message</b> : Inhouse Category deleted successfully</div>";
		}
		...
		if(isset($_POST['Submit']) || isset($_POST['Update']))
		{
				...
				if(isset($_POST['Submit']))
				{
					...
					{
						Inhousecategory_Insert();
						$message = "<br /><div class='message success'><b>Message</b> : Inhouse Category added successfully</div>";
					}
				}
				else if(isset($_POST['Update']))
				{
					...
					{
						Inhousecategory_Update();
						$message = "<br /><div class='message success'><b>Message</b> : Inhouse Category details updated successfully</div>";
					}
				}
			foreach($Columns as $Col)
				$_POST[$Col] = "";
	...
	<div class="columns" style='width:902px;'>
		...
		<form method="post" action="?page=<?php echo $_GET['page']."&subpage=".$_GET['subpage']."&innersubpage=".$_GET['innersubpage']."&pageno=".$_GET['pageno']; ?>" id="form" class="form panel" onsubmit="return validation()">
			...
			<header><h2>Add Inhouse Category</h2></header>
			<hr />				
			<fieldset>
				<div class="clearfix">
					<label>Inhouse Category <font color="red">*</font></label>
					<input type="text" autocomplete="off" id="productcategory" name="productcategory" required="required" value="<?php echo $_POST['name']; ?>" onkeypress="return isAlphabetic(event)"/>
				</div>
			</fieldset>
			<hr />
			<?php
			...
			<table class="paginate sortable full">
				<thead>
					<tr>
						...
						<th align="left">Inhouse Category</th>
						<th align="left">Action</th>
					</tr>
				</thead>
				<tbody>
					<?php
					if(!$InhousecategoryTotalRows['total'])
						echo '<tr><td colspan="3"><font color="red"><center>No data found</center></font></td></tr>';
					$Limit = 10;
					$total_pages = ceil($InhousecategoryTotalRows['total'] / $Limit);
					if(!$_GET['pageno'])
						$_GET['pageno'] = 1;
					...
					while($Inhousecategory = mysqli_fetch_assoc($InhousecategoryRows))
					{
						echo "<tr style='valign:middle;'>
							...
							<td>".$Inhousecategory['name']."</td>
							<td ><a href='index.php?page=".$_GET['page']."&subpage=".$_GET['subpage']."&innersubpage=".$_GET['innersubpage']."&id=".$Inhousecategory['id']."&pageno=".$_GET['pageno']."&action=Edit' class='action-button' title='user-edit'><span class='user-edit'></span></a>  &nbsp; <a href='#' onclick='deleterow(".$Inhousecategory['id'].")' class='action-button' title='user-delete'><span class='user-delete'></span></a></td>
						</tr>";
					} ?>
				</tbody>
			</table>
...
<script>
	...
	{
		var message = "";
		if(document.getElementById("productcategory")==0)
