		...
		if(!$_GET['WorkId'])
		{
			...
			?>
				<select id="workid" name="workid"  onchange="GetWorkDetails(this.value)">
					...
					<?php
						$A = array();
						while($FetchWork = mysql_fetch_array($SelectWork))
						{
							...
							//{
								///if($_GET['WorkId']==$FetchWork['id'])
								 ...
								 //if(mysql_num_rows(mysql_query("select oppurtunity_id from sales_order where oppurtunity_id='".$FetchWork['id']."'")))
