		...
		<div class="grid_6 first">
			<form method="post" action="?page=<?php echo $_GET['page']; ?>" id="form" class="form panel" onsubmit="return validation()">
				...
				<fieldset>
					...
					<div class="clearfix">
                        <label>Name</label>
						<input type="hidden" id="name" name="name" required="required" value="<?php echo $User['name']; ?>" onkeypress="return isAlphabetic(event)" />
						<?php echo $User['name']; ?>
					</div>
					<div class="clearfix">
                        <label>Current-Password <font color="red">*</font></label>
						<input type="password" id="password" name="password" required="required" value="" onkeypress="return isAlphabetic(event)" />
                    </div>
					<div class="clearfix">
                        <label>New-Password</label>
						<input type="password" id="newpassword" name="newpassword" value="<?php echo $User['newpassword']; ?>" onkeypress="return isAlphabetic(event)"/>
                    </div>
					<div class="clearfix">
                        <label>Confirm-Password</label>
						<input type="password" id="confirmpassword" name="confirmpassword" value="<?php echo $User['confirmpassword']; ?>" onkeypress="return isAlphabetic(event)"/>
                    </div>
					<div class="clearfix">
                        <label>Phone No. <font color="red">*</font></label>
						<input type="text" id="phone" name="phone" maxlength="25" required="required" value="<?php echo $User['phone']; ?>" onkeypress="return isNumeric(event)"/>
                    </div>
					<div class="clearfix">
						<label>Role</label>
						<?php
...
<script>
	...
	function isNumeric(evt)
	{
		...
		if(charCode == 8 || charCode ==45 || charCode ==47)
			return true;
	...
	{
		if(document.getElementById("phone").value.length < 26)
			return true;
		else
			return false;
	}
