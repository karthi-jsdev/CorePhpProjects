...
if(isset($_GET['vendorid'])||isset($_GET['date'])||isset($_GET['materialcode'])||isset($_GET['batchnumber'])||isset($_GET['quantity'])||isset($_GET['unitprice'])||isset($_GET['amount'])||isset($_GET['taxid'])||isset($_GET['taxamount'])||isset($_GET['number']) || isset($_GET['excise']) || isset($_GET['excises']) || isset($_GET['ecisetax1']) || isset($_GET['ecisetax2']) || isset($_GET['ecisetax3'])|| isset($_GET['couriers']))
{
	...
	{
		$invoiceid = mysql_fetch_assoc(Invoice_Selection_ForStock());
		...
		if($_POST['batchnumber'] == $batch_id['number'])
		{
			...
			$stock_batchid = $batch_idd = mysql_fetch_assoc(Batch_id_Selection());
			$stockbatchid = mysql_fetch_assoc(mysql_query("SELECT * FROM stock where batchid in(select batch.id from batch where batch.number='".$_POST['batchnumber']."' && rawmaterialid='".$_POST['materialcode']."')"));
			if($stockbatchid['batchid'])
			{
				...
				mysql_query("update stock set quantity='".$_POST['quantity']."',unitprice='".$_POST['unitprice']."',
		...
		{
			Batch_Insertion();
			$batch_id = $batch = mysql_fetch_assoc(Batchid_Recent()); 
				$_POST['batchid'] = $batch_id['id'];
				Stock_Inventory_Insertion();
			$stock_batchid = $batch_idd = mysql_fetch_assoc(Batch_id_Selection());
			$stockbatchid = mysql_fetch_assoc(mysql_query("SELECT * FROM stock where batchid in(select batch.id from batch where batch.number='".$_POST['batchnumber']."' && rawmaterialid='".$_POST['materialcode']."')"));
	...
	if($_SESSION['check']=="")
	{
		...
		else
		{
			...
			$invid = mysql_fetch_assoc(mysql_query("select * from invoice ORDER BY id DESC LIMIT 0,1"));
			if($invid['excise']==0)
			{}
			else
			{
				...
				mysql_query("UPDATE invoice,(select sum(amount) as amt from stockinventory where invoiceid ='".$s['invoiceid']."' group by invoiceid)as s SET exciseamount=((s.amt*('".$_POST['ecisetax1']."'+'".$_POST['ecisetax2']."'+'".$_POST['ecisetax3']."'))/'100') WHERE invoice.id='".$s['invoiceid']."'");
			}
	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				//mysql_query("UPDATE invoice SET exciseamount=(('".$s['amt']."'*'10.36')/'100') WHERE invoice.id='".$s['invoiceid']."'");
				$_SESSION['check'] = $_GET['randomnumber'];
			}
		}
	}
	...
	$batch_id = $batch = mysql_fetch_assoc(Batch_Selection());
		$_POST['batchid'] = $batch_id['id'];
	$d = mysql_fetch_assoc(Stock_Value_Batching());
		$_POST['total'] = $d['total'];		
		$_POST['unitt'] = $d['unitt'];		
		$_POST['amt'] =  $d['amt'];		
		$_POST['taxid'] =  $d['taxid'];		
		$_POST['taxamt'] =  $d['taxamt'];
	//$batchid = $batch = mysql_fetch_assoc(mysql_query("SELECT * FROM batch where batch.number = '".$_POST['batchnumber']."'"));
	/*$stockbatchid = mysql_fetch_assoc(mysql_query("SELECT * FROM stock where batchid in(select batch.id from batch where batch.number='".$_POST['batchnumber']."')"));
	if($stockbatchid['batchid'])
	{
		$_POST['quantity'] = $stockbatchid['quantity']+$_POST['quantity'];
		$_POST['unitprice'] = $stockbatchid['unitprice']+$_POST['unitprice'];
...
//	else
	//{//echo "insert into stock (batchid,quantity,unitprice,amount,taxid,taxamount) values ('".$_POST['batchid']."','".$_POST['quantity']."','".$_POST['unitprice']."','".$_POST['amount']."','".$_POST['taxid']."','".$_POST['taxamount']."')";
		//mysql_query("insert into stock (batchid,quantity,unitprice,amount,taxid,taxamount) values ('".$_POST['batchid']."','".$_POST['quantity']."','".$_POST['unitprice']."','".$_POST['amount']."','".$_POST['taxid']."','".$_POST['taxamount']."')");
	//}						
//$invoice = Invoice_Stock_Selection();			
/* if(!mysql_num_rows($invoice)>0)
{ */
