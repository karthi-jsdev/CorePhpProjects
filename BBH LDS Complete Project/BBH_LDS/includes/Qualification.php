<section role="main" id="main">
	<?php
		...
		if($_GET['action'] == 'Edit')
		{
			$Class = mysqli_fetch_assoc(Qualification_Select_ById());
		...
		else if($_GET['action'] == 'Delete')
		{
			...
			$message = "<br /><div class='message success'><b>Message</b> : One Qualification deleted successfully</div>";
		}
		
		if(isset($_POST['Submit']) || isset($_POST['Update']))
		{
			$ClassResource = Qualification_Select_ByNamePWD();
			if(isset($_POST['Submit']))
			{
				if(mysqli_num_rows($ClassResource))
					$message = "<br /><div class='message error'><b>Message</b> : This Qualification already exists</div>";
				else
				{
					Qualification_Insert();
					$message = "<br /><div class='message success'><b>Message</b> : Qualification added successfully</div>";
				}
			}
			else if(isset($_POST['Update']))
			{
				$Class = mysqli_fetch_assoc($ClassResource);
				if(mysqli_num_rows(Qualification_Select_ByNamePWDId()))
					$message = "<br /><div class='message error'><b>Message</b> : This Qualification already exists</div>";
				else
				{
					Qualification_Update();
					$message = "<br /><div class='message success'><b>Message</b> : Qualification details updated successfully</div>";
				}
			}
			foreach($Columns as $Col)
				$_POST[$Col] = "";
	...
	<div class="columns" style='width:902px;'>
		...
		<div class="columns">
			<h3>Qualification List
				<?php
				$QualificationTotalRows = mysqli_fetch_assoc(Qualification_Select_Count_All());
				echo " : No. of total Qualification - ".$QualificationTotalRows['total'];
				?>
			</h3>
			<hr />			
			<table class="paginate sortable full">
				...
				<tbody>
					...
					while($Qualification = mysqli_fetch_assoc($ClassRows))
					{
						echo "<tr style='valign:middle;'>
							<td align='center'>".$i++."</td>
							<td>".$Qualification['name']."</td>
							<td ><a href='index.php?page=".$_GET['page']."&subpage=".$_GET['subpage']."&innersubpage=".$_GET['innersubpage']."&id=".$Qualification['id']."&pageno=".$_GET['pageno']."&action=Edit' class='action-button' title='user-edit'><span class='user-edit'></span></a>  &nbsp; <a href='#' onclick='deleterow(".$Qualification['id'].")' class='action-button' title='user-delete'><span class='user-delete'></span></a></td>
						</tr>";
					} ?>
				</tbody>
			</table>
...
<script>
	...
	function validation()
	{
		...
		if(document.getElementById("name").value.length < 2 || document.getElementById("name").value.length > 15)
			message = "Qualification name should be within 2 to 15 characters";
