<?php
	...
	{
		$Top_Quotationtotalrows = mysql_fetch_assoc(Top_Quotationrows());
		$Limit = 5;
		...
		while($Current_MonthQuotation = mysql_fetch_assoc($Top_Quotation))
		{
			$Status = mysql_fetch_assoc(Select_Status_By_Quotation_Id($Current_MonthQuotation['id']));
	...
	{
		$Quotationpendingrows = mysql_fetch_assoc(pendingrows());
		$Limit = 5;
		...
		while($Current_MonthPending = mysql_fetch_assoc($Pending_Quotation))
		{
			$Pending_Status = mysql_fetch_assoc(Select_Pending_Status_Quotation_Id($Current_MonthPending['id']));
	...
	{
		$QuotationStatus = mysql_fetch_assoc(QuotationStatusrows());
		...
		while($Current_MonthStatus = mysql_fetch_assoc($CurrentmonthQuotationStatus))
	...
	{
		$Numberofquotation = mysql_fetch_assoc(Numberofquotationrows());
		...
		while($NumberOfQuotation_Month = mysql_fetch_assoc($NumberofquotationStatus))
