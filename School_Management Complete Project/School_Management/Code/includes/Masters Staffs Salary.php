					...
					while($Salary = mysql_fetch_assoc($SalaryRows))
					{
						echo "<tr style='valign:middle;'>
							...
							if($Salary['isdeduction'])
								...
								echo "No";
						echo "</td><td ><a href='index.php?page=".$_GET['page']."&subpage=".$_GET['subpage']."&innersubpage=".$_GET['innersubpage']."&id=".$Salary['id']."&pageno=".$_GET['pageno']."&action=Edit' class='action-button' title='user-edit'><span class='user-edit'></span></a>  &nbsp; <a href='#' onclick='deleterow(".$Salary['id'].")' class='action-button' title='user-delete'><span class='user-delete'></span></a></td>
						</tr>";
					} ?>
				</tbody>
			</table>
...
<script>
	...
	function validation()
	{
		...
		if(document.getElementById("particular").value.length < 2 || document.getElementById("particular").value.length > 15)
			message = "Particulars should be within 2 to 15 characters";
		if(message)
		{
			alert(message);
			return false;
