			...
			<tr>
				<td>
					...
					<select name="name" id="name">	
						...
						<?php
						$SelectStaffName = mysqli_query($_SESSION['connection'],"select * from staff_details  group by name");
						while($FetchStaffName = mysqli_fetch_array($SelectStaffName))
						{
				...
				<td id="sec">
					...
					<select name="section" id="section">
						<option value="">All</option>
						<?php
						$SelectSection = mysqli_query($_SESSION['connection'],"select * from section order by name asc");
...
<?php
	if($_POST['Search'])
	{
		...
		$SelectEmployee = mysqli_query($_SESSION['connection'],"SELECT staff_details.name as StaffName,section.name as SectionName, class.name as ClassName, staff_details.subject_ids as SubjectIds From staff_details JOIN section ON section.id=staff_details.section_id JOIN class ON class.id = classid ".str_replace("=''", "!=''", $Query));
		echo'<table class="paginate sortable full" style="width:800px">
			...
			$Subjects = array();
				while($FetchEmployee = mysqli_fetch_array($SelectEmployee))
				{
					$SubjectNames = explode(',',$FetchEmployee['SubjectIds']);
					foreach($SubjectNames as $SubjectName)
					{
						$Sub = mysqli_fetch_array(mysqli_query($_SESSION['connection'],"select name from subject where id='".$FetchEmployee['SubjectIds']."'"));
						$Subjects[] = $Sub['name'];
					}
					echo '<tr>
