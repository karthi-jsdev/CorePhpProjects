<section role="main" id="main">
	<?php
		...
		{
			Category_Delete_ById($_GET['id']);
			$message = "<br /><div class='message success'><b>Message</b> : One Category name deleted successfully</div>";
		}
		...
		if(isset($_POST['Submit']) || isset($_POST['Update']))
		{
			...
			if(isset($_POST['Submit']))
			{
				...
				{
					Category_Insert();
					$message = "<br /><div class='message success'><b>Message</b> : Category name added successfully</div>";
				}
			}
		...
		<form method="post" action="?page=<?php echo $_GET['page']."&subpage=".$_GET['subpage']."&pageno=".$_GET['pageno']; ?>" id="form" class="form panel" onsubmit="return validation()">
			...
			<fieldset>
				<div class="clearfix">
					...
					<input type="text" id="categoryname" name="categoryname" required="required" value="<?php echo $_POST['name']; ?>" onkeypress="return AlphaNumCheck(event)"/>
				</div>
			</fieldset>
			<hr />
			<?php
		...
		<div class="columns">
			...
			<table class="paginate sortable full">
				...
				<tbody>
					...
					if($_GET['pageno']>1)
						$i = $CategoryTotalRows['total']- $Start;
					else
					...
					while($Category = mysql_fetch_assoc($CategoryRows))
					{
						echo "<tr style='valign:middle;'>
							...
							<td>".$Category['name']."</td>
							<td ><a href='index.php?page=".$_GET['page']."&subpage=".$_GET['subpage']."&id=".$Category['id']."&pageno=".$_GET['pageno']."&action=Edit' class='action-button' title='user-edit'><span class='user-edit'></span></a>  &nbsp; <a href='#' onclick='deleterow(".$Category['id'].")' class='action-button' title='user-delete'><span class='user-delete'></span></a></td>
						</tr>";
					} ?>
				</tbody>
			</table>
