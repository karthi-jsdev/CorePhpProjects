<section role="main" id="main">
	<?php
		...
		if($_GET['action'] == 'Edit')
		{
			$Class = mysqli_fetch_assoc(Deparment_Select_ById());
		...
		if(isset($_POST['Submit']) || isset($_POST['Update']))
		{
			...
			else if(isset($_POST['Update']))
			{
				...
				if(mysqli_num_rows(Deparment_Select_ByNamePWDId()))
					$message = "<br /><div class='message error'><b>Message</b> : This Deparment already exists</div>";
				else
				{
					Deparment_Update();
	...
	<div class="columns" style='width:902px;'>
		...
		<div class="columns">
			<h3>Deparment List
				<?php
				$DeparmentTotalRows = mysqli_fetch_assoc(Deparment_Select_Count_All());
				echo " : No. of total Users - ".$DeparmentTotalRows['total'];
				?>
			</h3>
			<hr />			
			<table class="paginate sortable full">
				...
				<tbody>
					...
					if($_GET['pageno']>1)
						$i = $DeparmentTotalRows['total']- $Start;
					else
						$i = $DeparmentTotalRows['total'];
					$Status = array("<a href='#' class='action-button' title='delete'><span class='delete'></span></a>", "<a href='#' class='action-button' title='accept'><span class='accept'></span></a>");
					$ClassRows = Deparment_Select_ByLimit($Start, $Limit);
					while($Deparment = mysqli_fetch_assoc($ClassRows))
					{
						echo "<tr style='valign:middle;'>
							<td align='center'>".$i--."</td>
							<td>".$Deparment['name']."</td>
							<td ><a href='index.php?page=".$_GET['page']."&subpage=".$_GET['subpage']."&id=".$Deparment['id']."&pageno=".$_GET['pageno']."&action=Edit' class='action-button' title='user-edit'><span class='user-edit'></span></a>  &nbsp; <a href='#' onclick='deleterow(".$Deparment['id'].")' class='action-button' title='user-delete'><span class='user-delete'></span></a></td>
						</tr>";
					} ?>
				</tbody>
			</table>
