<section role="main" id="main">
	<?php
		...
		if($_GET['action'] == 'Edit')
		{
			$Class = mysqli_fetch_assoc(Occupation_Select_ById());
		...
		if(isset($_POST['Submit']) || isset($_POST['Update']))
		{
			...
			else if(isset($_POST['Update']))
			{
				...
				if(mysqli_num_rows(Occupation_Select_ByNamePWDId()))
					$message = "<br /><div class='message error'><b>Message</b> : This Occupation already exists</div>";
				else
				{
					Occupation_Update();
	...
	<div class="columns" style='width:902px;'>
		...
		<div class="columns">
			<h3>Occupation List
				<?php
				$OccupationTotalRows = mysqli_fetch_assoc(Occupation_Select_Count_All());
				echo " : No. of total Users - ".$OccupationTotalRows['total'];
				?>
			</h3>
			<hr />			
			<table class="paginate sortable full">
				...
				<tbody>
					...
					if($_GET['pageno']>1)
						$i = $OccupationTotalRows['total']- $Start;
					else
						$i = $OccupationTotalRows['total'];
					$Status = array("<a href='#' class='action-button' title='delete'><span class='delete'></span></a>", "<a href='#' class='action-button' title='accept'><span class='accept'></span></a>");
					$ClassRows = Occupation_Select_ByLimit($Start, $Limit);
					while($Occupation = mysqli_fetch_assoc($ClassRows))
					{
						echo "<tr style='valign:middle;'>
							<td align='center'>".$i--."</td>
							<td>".$Occupation['name']."</td>
							<td ><a href='index.php?page=".$_GET['page']."&subpage=".$_GET['subpage']."&id=".$Occupation['id']."&pageno=".$_GET['pageno']."&action=Edit' class='action-button' title='user-edit'><span class='user-edit'></span></a>  &nbsp; <a href='#' onclick='deleterow(".$Occupation['id'].")' class='action-button' title='user-delete'><span class='user-delete'></span></a></td>
						</tr>";
					} ?>
				</tbody>
			</table>
