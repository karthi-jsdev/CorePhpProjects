...
<form method="POST" name="form1" action="?page=<?php echo $_GET['page']."&subpage=".$_GET['subpage']."&pageno=".$_GET['pageno']; ?>" id="form" class="form panel" onsubmit="return validate();">
	<fieldset>
		...
		<div class="clearfix">
			<label><strong>Class & Section </strong><font color="red">*</font></label>
...
<?php
	if(($_GET['name'] && $_GET['Section']) || $_GET['number'] || $_GET['Section'])
	{
		...
		{
			echo "select * from student_admission where section_id='".$_GET['Section']."'";
		...
		if(mysql_num_rows($FetchStudent))
		{
			while($SelectStudent = mysql_fetch_array($FetchStudent))
			{
				...
				echo '<tr>
						...
						<td style="vertical-align:middle"><a href="index.php?page='.$_GET['page'].'&subpage=Student_Admission&id='.$SelectStudent['id'].'&pageno='.$_GET['pageno'].'&action=Edit">Edit</a><br/>
...
<script>
	function Search()
	{
		...
		{
			message = "Please Enter Any of Two Fields";
		}
		if(!Section && name)
