<?php
	...
	$sql1 = mysql_query("SELECT * FROM lead WHERE ptclid NOT IN (SELECT ptclid FROM comments)");
	//$row_status =mysql_fetch_array(mysql_query("select * from comments where status_id='".$_GET['id']."'"));
	...
	if(mysql_num_rows($sql1))
	{
		...
		<table  border='1'  align='left' class='paginate sortable full'>
			...
			{	
				$sql = mysql_query("SELECT * FROM lead WHERE ptype='".$prod."' AND ptclid NOT IN (SELECT ptclid FROM comments) ORDER BY PTCLID DESC");
