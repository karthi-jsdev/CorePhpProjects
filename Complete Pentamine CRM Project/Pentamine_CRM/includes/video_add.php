...
{
	$id1=intval(base64_decode($_GET['videoId']));
	$cat=intval(base64_decode($_GET['cat']));
	$query = " WHERE clientId='".$_SESSION['clientId']."' AND categoryId=$cat";
	$sql_faq="select * from video where clientId='".$_SESSION['clientId']."' AND videoId=$id1";
...
$sql = "SELECT max(sortOrder) AS sortorder FROM video $query";
...
{ //echo '<pre>'; print_r($_REQUEST); echo '</pre>'; exit;
	...
	$sql="select count(*) from video where clientId='".$_SESSION['clientId']."' AND title='$faq_question'";
	$res=mysql_query($sql);
...
{  //echo '<pre>'; print_r($_REQUEST); echo '</pre>'; exit;
	...
	{ 
		$sql = "SELECT * FROM video WHERE clientId='".$_SESSION['clientId']."' AND videoId=$faq_id";
		$result = mysql_query($sql);
...
if(count($error_Msg)==0)
{
	...
	$arFieldsValues['sortOrder'] = $sortorder;
	$arFieldsValues['duration'] = $duration;

   	if($_REQUEST["Save"])
   	{	
		
		$sql_sorting = "SELECT * FROM video WHERE clientId='".$_SESSION['clientId']."' AND sortOrder='$sortorder' AND categoryId='$cat'";
		$result_sorting= $db->select($sql_sorting);
		$count_sort=count($result_sorting);
		if($count_sort>0) 
		{ 	
			$sql_sorting_new = "SELECT * FROM  video WHERE clientId='".$_SESSION['clientId']."' AND sortOrder>='$sortorder' AND categoryId='$cat' ORDER BY photo DESC";
			$result_sorting_new= $db->select($sql_sorting_new);
			foreach($result_sorting_new as $key=> $data) 
			{	
				 $arFieldsValuesz['sortOrder'] = $data['sortOrder'] + 1;
				 $resulter = $db->update('video',$arFieldsValuesz,'videoId='.$data['videoId']);
			}
		}
		...
		$sql = "SELECT * FROM video_subscribe WHERE clientId='".$_SESSION['clientId']."' AND status=1";
		$result = mysql_query($sql);
	...
	{		
		...
		{
		$sql_sorting_new1="SELECT * FROM  video  WHERE clientId='".$_SESSION['clientId']."' AND sortOrder<='$sortorder' AND sortOrder >='$oldorder' AND categoryId='$cat'  ORDER BY sortOrder DESC";
		$result_sorting_new1= $db->select($sql_sorting_new1);
			foreach($result_sorting_new1 as $key=> $data1) 
			{	
				$arFieldsValueszv['sortOrder'] = $data1['sortOrder'] - 1;
				$resulter1=$db->update('video',$arFieldsValueszv,'videoId='.$data1['videoId']);
			}
		} 
		...
		$result=$db->update('video',$arFieldsValues,'videoId='.$id1);
		header("location:videos_list.php?action=update&maxrows=$maxrows&keywords=$keywords&display=$display&search=$search&main=$main&catid=$catid");
		exit;
	}
    ...
    <div class="columns">
		...
		<input name="oldsortorder" type="hidden" id="oldsortorder" value="<?php echo $sortorder; ?>">
                <fieldset>
		        	...
		        	<?php
					$sql = "SELECT * FROM video_category ORDER BY  `video_category`.`categoryName` ASC";
					$result = mysql_query($sql);
					while ($tmp = mysql_fetch_assoc($result)) 
					{
				?>
				  <option value="<?php echo base64_encode($tmp[categoryId]); ?>" <?php if ($cat == $tmp[categoryId]) { ?> selected <?php } ?>><?php echo $tmp['categoryName']; ?></option>
				  <?php 
				  }
				  ?>
		        	</select>
                    	...
                    	<div class="clearfix">
                        	...
                        	<input name="faq_question" type="text" id="faq_question" value="<?php echo stripslashes($faq_question); ?>" size="42" />
                        	<input name="faq_id" type="hidden" id="faq_id" value="<?php echo base64_encode($id1);?>">
                    ...
                    <div class="clearfix">
                        ...
                        <textarea name="faq_answer" cols="41" rows="3" id="faq_answer"><?php echo $faq_answer;?></textarea>
                    </div>
                    <div class="clearfix">
					...
					<div class="clearfix" id="bfile" style="display: <?php if ($data['type'] == 1) echo "none"; else echo "block";?>"  >
                    ...
                    <div id="ufile" style="display:<?php if ($data['type'] == 1) echo "block"; else echo "none"; ?>" >
		            <div class="clearfix">
